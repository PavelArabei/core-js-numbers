...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const firstSquare = (x2 - x1) ** 2;
  const secondSquare = (y2 - y1) ** 2;
...
function roundToPowerOfTen(num, pow) {
  const powScale = 10 ** pow;
  return Math.round(num / powScale) * powScale;
}
